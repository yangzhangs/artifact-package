...
- <mark>Using a LLM or any tools based on LLM output is a mixed bag:</mark>
  - <mark>If your PR introduces code generated by a LLM but it's not noticeable because the quality is indistinguishable from an experienced dev, then that is fine.</mark>
  - <mark>If your PR is obviously just vibe coded, the PR will likely get rejected without a proper review. Limited maintainer time is better used elsewhere.</mark>
